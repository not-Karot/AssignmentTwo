...
def insertCompany(companyMetaData):
    try:
        ...
        sqlite_insert_with_param = """INSERT INTO company
                             (id, name, website, headquarter, employees, industry, type, source) 
                             VALUES (?, ?, ?, ?, ?, ?, ?, "investing");"""
...
def worker(start, end):
    ...
    links = pickle.load(open('investing_links_v1.p', 'rb'))
    while current < ending:
        ...
        try:
            companyMetadata = (
                current,
                driver.find_element_by_xpath('//*[@id="leftColumn"]/div[1]/h1').text,
        ...
        try:
            scrapeEmployees(driver.find_element_by_xpath('//*[@id="leftColumn"]/table/tbody'), current)
        except selenium.common.exceptions.NoSuchElementException as err:
            print(err)
            print(links[current])
            print(links[current][:pos] + "-company-profile/")
...
if __name__ == '__main__':
    ...
    #pickle.dump(links, open('investing_links_v1.p', 'wb'))
    #obj = pickle.load(open('investing_links_v1.p', 'rb'))
    startScraping(0, 1250, 5)
    #print(len(obj))
    #worker(0, 1250)
